...
### Copy General Source ---
Z=0
recent_monday_w=`date -d "Last monday - $Z weeks" +"%w"`;
today_w=`date +"%w"`;
if [ ${today_w} -eq 0 ] ;then
  today_w=7
fi
WW=`expr ${today_w} - ${recent_monday_w}`
# ---
numSUN1=`expr ${WW} + 8`
SUN1a=`date -d "${numSUN1} days ago - $Z weeks" +"%Y%m%d"`
# ---
numSUN2=`expr ${WW} + 1`
SUN2a=`date -d "${numSUN2} days ago - $Z weeks" +"%Y%m%d"`
FILEDATE="${SUN2a}"
# ----
numSAT2=`expr ${WW} + 2`
SAT2a=`date -d "${numSAT2} days ago - $Z weeks" +"%Y%m%d"`
# ---
LOGDIR="/usr/dfws/etc/Akamai_Programs/1170772/log"
OGKANGEN_DIR="logkangen"
FOLDERDATE="${SUN1a}-${SAT2a}"
WORKFILE1="alllog_win"
### Copy General Source ---

NO1_ARG=$1	# No.1 Arg. Specifically, ${WORKFILE1}.txt. But CRLF File.
LF_FILE=${NO1_ARG}.lf
...
function archive_and_remove_files() {
	zip ${LOGDIR}/${LOGKANGEN_DIR}/$FOLDERDATE/alllog_${FILEDATE}_by_folder_path.zip ./${BY_FOLDER_PATH_FILE}.txt
	rm ./${WORKFILE1}_akamai.txt
}
